...
public class SimpleDirectoryWatchService extends AbstractService implements DirectoryWatchService, Runnable,IjfxService {
    ...
    private Set<FileChangeListener> matchedListeners(Path dir, Path file) {
        return getListeners(dir)
                .stream()
                .filter((FileChangeListener listener) -> matchesAny(file, getPatterns(listener)))
        ...
        for (WatchEvent<?> event : key.pollEvents()) {
            ...
            if (eventKind.equals(ENTRY_CREATE)) {
                matchedListeners(getDirPath(key), file)
                        .parallelStream()
                        .forEach((FileChangeListener listener) -> listener.onFileCreate(file.toString()));
            } else if (eventKind.equals(ENTRY_MODIFY)) {
                matchedListeners(getDirPath(key), file)
                        .parallelStream()
                        .forEach((FileChangeListener listener) -> listener.onFileModify(file.toString()));
            } else if (eventKind.equals(ENTRY_DELETE)) {
                matchedListeners(getDirPath(key), file)
                        .parallelStream()
                        .forEach((FileChangeListener listener) -> listener.onFileDelete(file.toString()));
            }
        }
    }
