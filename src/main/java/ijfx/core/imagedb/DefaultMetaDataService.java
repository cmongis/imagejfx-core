...
public class DefaultMetaDataService extends AbstractService implements MetaDataService {
    ...
    public MetaDataSet extractMetaData(File file) {
        ...
        try {
            ...
            try {
                ...
                metadata = getSCIFIO().format().getFormatFromClass(BioFormatsFormat.class).createParser().parse(fileLocation);
            }
            if (metadata == null) {
    ...
    private ReaderFilter getReaderFilter(File file) throws FormatException, IOException,URISyntaxException {
        return scifio.initializer().initializeReader(locationService.resolve(file.getAbsolutePath()), config);
    }
