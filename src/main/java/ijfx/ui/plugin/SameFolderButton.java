...
@UiConfiguration(id = "same-folder-button", context = "imagej+image-open", localization = Localization.TOP_RIGHT)
public class SameFolderButton extends ToggleButton implements UiPlugin {

    @Parameter
    Context context;

    IconExplorerView iconView = new IconExplorerView();
    ...
    private PaneCell<Explorable> createIcon() {
        ...
        return new SimplePaneCell<Explorable>()
                ...
                .setWidth(120)
                .setOnDataClick(this::open);
    }

    private void open(DataClickEvent<Explorable> event) {
        
        Explorable explorable = event.getData();
        
        popOver.hide();

        explorerViewService.open(explorable);
    }
    ...
    private void update() {
        ...
        if (file.exists()) {
            ...
            List<File> files = new ArrayList(explorerService.getAllImagesFromDirectory(parent, false));
