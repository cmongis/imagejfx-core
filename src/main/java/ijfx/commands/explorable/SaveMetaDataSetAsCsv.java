 ...
 */
@Plugin(type = Command.class, label = "Export to CSV...", iconPath = "fa:save")
public class SaveMetaDataSetAsCsv extends ContextCommand {

    @Parameter(label = "Save to...", style = "save csv")
    File outputFile;

    @Parameter(label = "Include header")
    Boolean includeHeader = true;
    
    @Parameter(type = ItemIO.BOTH)
    MetaDataOwnerDisplay objectDisplay;
    ...
    public void run() {
        ...
        List<MetaDataSet> rows = objectDisplay
                .stream()
                .flatMap(owner -> owner.stream())
                .map(owner -> owner.getMetaDataSet())
                .collect(Collectors.toList());
        String content = MetaDataSetUtils.exportToCSV(rows, ",", true, MetaDataKeyPriority.getPriority(rows.get(0)));
        try {
